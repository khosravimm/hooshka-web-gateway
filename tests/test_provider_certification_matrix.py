...
def test_provider_certification_matrix_is_scoped_and_conservative():
    ...
    assert rows['deepseek-web']['status']=='E3_PASS_SCOPED'
    assert rows['deepseek-web']['evidence_level']=='E3_SCOPED'
...
def test_work013_remains_partial_until_other_scopes_and_interactive_e2_close():
    ...
    assert item['latest_evidence']['record']=='docs/evidence/HWG_2.1.0_E3_DEEPSEEK_W3_REPLACEMENT_20260925.md'
    assert 'exact scope' in item['latest_evidence']['result'] and 'broader provider certification remains open' in item['latest_evidence']['result']
