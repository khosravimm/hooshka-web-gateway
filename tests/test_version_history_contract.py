...
def test_release_version_sources_are_synchronized():
    ...
    assert version == "1.0.0-dev.4"
    assert manifest["version"] == version
    assert ui["version"] == version
    assert manifest["build_date"] == "2026-09-21"
    assert ui["released"] == "2026-09-21"


def test_dev4_change_history_and_schema_contracts_exist():
    changelog = (ROOT / "CHANGELOG.md").read_text(encoding="utf-8-sig")
    record = ROOT / "docs/governance/HWG_1.0.0-dev.4_CHANGE_RECORD_20260921.md"
    assert "## 1.0.0-dev.4 - 2026-09-21" in changelog
