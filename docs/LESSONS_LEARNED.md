...
37. **Provider enforcement is a terminal account-state, not a retryable transport error.** DeepSeek Web displayed a temporary suspension after a local PoC produced 23 completion requests before the suspension deadline, including short bursts. The exact trigger is not disclosed, so the correlation is recorded as a plausible contributing factor rather than fact. Any mute/suspension/challenge state must open a circuit breaker: no replay, no account/IP rotation, no CAPTCHA/WAF bypass, and no stress testing against personal Web accounts.
38. **Web-chat acceptance testing needs a risk budget.** Use concurrency 1, conservative spacing/jitter, bounded predefined E2 cases, session reuse, and immediate stop on provider risk-control signals. Sustained automation belongs on a provider-supported API when available and policy-compliant.

39. **Provider-owned proof can be delegated without copying secrets.** Z.ai showed a viable middle path between direct HTTP and DOM answer scraping: let the official frontend create `X-Signature` and CAPTCHA proof, but capture and parse the backend SSE response from the browser fetch stream. This preserves backend evidence while avoiding credential extraction and CAPTCHA bypass.
40. **A dedicated CDP profile is part of the provider runtime contract.** Service-level browser transports must verify `/json/version` for the intended Chrome instance and fail clearly when the CDP endpoint is absent or points to the wrong browser.
