# HWG Agent API Specification — نسخه 1.0.0-dev.1

- document_id: HWG-AGENT-API-SPEC-1
- version: 1.0.0-dev.1
- date: 2026-09-20
- status: DRAFT — در حال تدوین، بدون ادعای تضمین سازگاری
- supersedes: HWG_1.0_AGENT_API_SPEC_v1.0.0-dev.0.md (افزودن `POST /v1/responses`)
...
| `POST /v1/responses` | POST | پیاده‌سازی‌شده (dev.1، نگاشت normalized روی چت؛ بدون استریم) | Responses API برای Agent (استاندارد OpenAI) |
...
### ۴.۲ `POST /v1/responses` (جدید در dev.1)

```json
{"model": "chatgpt-web", "input": "Explain...", "provider": "chatgpt-web"}
```

- `input`: رشته، یا لیست قطعات (`text`/`input_text`)، یا لیست پیام‌ها (`role`+`content`).
- نگاشت normalized روی همان پایپ‌لاین چت (انتخاب پروایدر، slot، ترجمه، نرمال‌سازی، audit).
- خروجی `{object:"response", output:[{type:"message", role:"assistant", content:[{type:"output_text", text}]}]}`.
- `stream:true` ⇒ `400 unsupported_streaming` (استریم فقط از `/v1/chat/completions`).
- مدل/پروایدر ناشناس ⇒ `404 model_not_found` / `unknown_provider` (مشابه چت).

### ۴.۳ اشکال استاندارد خطا (پاس‌نشده در تولیدی، اجباری)
...
- dev.1: `POST /v1/responses` + تست‌های `tests/test_responses_api.py` (۴ تست).